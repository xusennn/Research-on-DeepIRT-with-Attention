...
class MemoryHeadGroup():
    ...
    def attention_weight(self, query_list, key_list,key_memory_state_dim):
        """
        ...
        """
        output = tf.matmul(query_list, tf.transpose(key_list, (0, 2,1)))/tf.math.sqrt(tf.cast(key_memory_state_dim,tf.float32))
    ...
    def write(self, value_memory_matrix, embedded_content_vector,embedded_query_vector, memory_matrix_pre_list, reuse=False):
        """
        ...
        """
        # print('mvt_list.shape')
        # print(memory_matrix_pre_list[0].shape)
        
        # print('mvt.shape')
        # print(value_memory_matrix.shape)
        ...
        last_value_memory_matrix = value_memory_matrix

        embedded_content_vector, value_memory_matrix = self.mogrifer(embedded_content_vector, value_memory_matrix, self.delta_1, self.delta_1, self.rounds)
        ...
        erase_signal = tf.sigmoid(self.linear_lstm(embedded_content_vector, value_memory_matrix_reshaped_hpre,
                                self.memory_state_dim, bias=True, scope=self.name + '/EraseOperation', ))
        
        zt_signal = tf.sigmoid(self.linear_lstm(embedded_content_vector, value_memory_matrix_reshaped_hpre,
                              self.memory_state_dim, bias=True, scope=self.name + '/ZtOperation', ))
        ...
        embedded_query_vector = layers.fully_connected(
            inputs=embedded_query_vector,
            num_outputs=self.memory_size,
            scope=self.name + '/AttentionOperation',
            reuse=reuse,
            activation_fn=None
        )


        embedded_query_vector = tf.nn.softmax(embedded_query_vector)
        ...
        cw_reshaped = tf.reshape(embedded_query_vector, [-1, self.memory_size, 1])
...
class DKVMN():
    ...
    def self_attention(self,qt, embedded_query_vector,reuse):
        """

        Args:
            embedded_query_vector: Shape: (BS x window_size x dim)(32 x n x 50); order: (qt-n ... qt-1 -> qt)
            qt: Shape:(32 x 50)

        Returns:
            attention_weight: Shape: (BS x window_size)
        """
        key = layers.fully_connected(
            inputs=qt,
            num_outputs=self.key_memory_state_dim,
            scope=self.name + '/Attentionkey',
            reuse=reuse,
            activation_fn=tf.tanh
        )
        query = layers.fully_connected(
            inputs=embedded_query_vector,
            num_outputs=self.key_memory_state_dim,
            scope=self.name + '/Attentionquery',
            reuse=reuse,
            activation_fn=tf.tanh
        )

        key = tf.reshape(key,[-1,1,self.key_memory_state_dim])

        attention_weight = self.key_head.attention_weight(query, key,self.key_memory_state_dim)
        attention_weight = tf.squeeze(attention_weight, 2)
        tf.nn.softmax(attention_weight, axis = 1)
        return attention_weight

    def write(self, embedded_result_vector,embedded_query_vector, memory_matrix_pre_list, reuse):
