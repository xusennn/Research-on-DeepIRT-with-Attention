...
class DeepIRTModel_skill(object):
    ...
    def _influence(self):
        ...
        for i in range(self.args.seq_len):
            ...
            if i>=self.args.window_size:
              window = sliced_q_embed_data[i-self.args.window_size+1:i+1]
            else:
              window = sliced_q_embed_data[0:i+1]
            new_window = None
            ...
            self.attention_weight_1 = self.memory.self_attention(q,embedded_query_vector=window,reuse = reuse_flag)
            ...
            self.new_memory_value, memory_matrix_pre = self.memory.write( qa,q, memory_matrix_pre_list, reuse=reuse_flag)
    ...
    def _create_loss(self):
        ...
        logger.info("filtered_logits: {}".format(filtered_logits))
        ...
        logger.info("loss: {}".format(self.loss))

    def _create_optimizer(self):
        with tf.variable_scope('Optimizer'):
            self.optimizer = tf.train.AdamOptimizer(learning_rate=self.args.learning_rate)
            gvs = self.optimizer.compute_gradients(self.loss)
            logger.info("gvs: {}".format(gvs))
